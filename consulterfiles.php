...
// }
$month = isset($_POST['month']) ? $_POST['month'] : 1; // Default to current month if none selected


function fetchTableData($pdo, $tableName, $month) {
    $stmt = $pdo->prepare("SELECT * FROM `$tableName` WHERE MONTH(MOIS) = :month");
...
<main id="main-container">
    ...
    <form method="POST" action="">
        <select name="month" onchange="this.form.submit()">
            <?php
            // Default to January if no month is selected, otherwise use the selected month
            ...
            <?php foreach ($tables as $index => $table): ?>
                <div class="tab-pane <?= $index === 0 ? 'active' : '' ?>" id="<?= $table ?>">
                    ...
                    <div class="table-responsive">
                        <?php
                        $tableData = fetchTableData($pdo, $table, $month);
