...
// Processing General Files //Collaborators & Production & Direct Charges
if (isset($_FILES['collaborators'], $_FILES['production'], $_FILES['charges'])) {
...

    {
        ...
        $command = escapeshellcmd("python3 verify_general_files.py " . escapeshellarg($host) . " " . escapeshellarg($db_name) . " " . escapeshellarg($username) . " " . escapeshellarg($password) . " " . escapeshellarg($collaboratorsPath) . " " . escapeshellarg($productionPath) . " " . escapeshellarg($chargesPath) . " " . escapeshellarg($refFilePath));
        $output = shell_exec($command);
        $result = json_decode($output, true);
    ...
    if (isset($_FILES['incharges'])) {
        ...
        echo $output;  // Display output from Python script for debugging
    }
